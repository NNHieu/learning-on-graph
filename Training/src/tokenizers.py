...
# from .numeral_tokenizer import NumeralTokenizer
...
class NumeralTokenizer:
    def __init__(self, num_nodes):
        ...
        vocab = [str(i) for i in range(num_nodes)]
        # vocab += ['|', '=', '/', '$', '?', '&', '*', ]
        vocab += [c for c in r'|=/$?&*-+><[](){};.']
        self.encoder = {c: i for i,c in enumerate(vocab)}
        self.encoder[':'] = -1

        self.decoder = {id:c for c, id in self.encoder.items()}
...
class LegoTokenizer:
    def __init__(self):
        # self.num_nodes = num_nodes
        alphabet = "abcdefghijklmnopqrstuvwxyz+-1|=?/$:TF"
        # Define encoder and decoder as a dictionary
        self.encoder = {c: i for i,c in enumerate(alphabet)}
        self.decoder = {id:c for c, id in self.encoder.items()}
...
class GPT2NumeralTokenizer(PreTrainedTokenizer):
    ...
    def convert_tokens_to_string(self, tokens):
        ...
        tokens = list(map(lambda x: "<|unk|>" if x is None else x, tokens))
        text = ",".join(tokens)
        text = text.replace(",|,", "|")
        text = text.replace(",/,", "/")
        text = text.replace(",=", "=")
...
class GPT2LegoTokenizer(PreTrainedTokenizer):
    vocab_files_names = VOCAB_FILES_NAMES
    model_input_names = ["input_ids", "attention_mask"]
...
if __name__ == "__main__":
    ...
    print(tokenizer.all_special_ids)
    print(tokenizer.added_tokens_encoder)
    enc = tokenizer(["d=-b|f=-g|g=-e|a=+f|h=-c|i=-d|e=-c|b=+h/c=+1,i=?h=-c=-1,b=+h=-1,d=-b=+1,i=-d=-1",
                     "x=-b|d=-b|f=-g|g=-e|a=+f|h=-c|i=-d|e=-c|b=+h/c=+1,i=?h=-c=-1,b=+h=-1,d=-b=+1,i=-d=-1"])
    print(enc)
    paded_enc = tokenizer.pad(enc)
    print(paded_enc)
    dec = tokenizer.decode(paded_enc["input_ids"][1])
